...
class SudokuServer : noncopyable {
 public:
  SudokuServer(EventLoop* loop, const InetAddress& listenAddr,
      ...
      : server_(loop, listenAddr, "SudokuServer"),
        ...
        stat_(threadPool_),
        inspectThread_(),
        inspector_(inspectThread_.startLoop(), InetAddress(9982),
                   "sudoku-solver") {
 ...
 private:
  void onConnection(const TcpConnectionPtr& conn) {
    ...
    if (conn->connected() && tcpNoDelay_) conn->setTcpNoDelay(true);
  }
  ...
  void solve(const TcpConnectionPtr& conn, const Request& req) {
    ...
    stat_.recordResponse(Timestamp::now(), req.receiveTime,
                         result != kNoSolution);
  }

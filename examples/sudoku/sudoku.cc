...
class SudokuSolver {
 public:
  SudokuSolver(int board[kCells]) : inout_(board), curr_node_(0) {
    stack_.reserve(100);

    root_ = new_column();
    root_->left = root_->right = root_;
    std::memset(columns_, 0, sizeof(columns_));
 ...
 private:
  ...
  Column* new_column(int n = 0) {
    assert(curr_node_ < kMaxNodes);
    Column* c = &nodes_[curr_node_++];
    std::memset(c, 0, sizeof(Column));
  ...
  Node* new_row(int col) {
    ...
    Node* r = &nodes_[curr_node_++];

    // Node* r = new Node;
    //memZero(r, sizeof(Node));
    std::memset(r, 0, sizeof(Node));
    r->left = r;
    r->right = r;
    r->up = r;
    r->down = r;

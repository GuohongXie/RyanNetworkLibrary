...
const char* GetErrnoMsg(int saved_errno) {
    return ::strerror_r(saved_errno, thread_info::t_errnobuf, sizeof(thread_info::t_errnobuf));
}
...
Logger::Impl::Impl(Logger::LogLevel level, int saved_errno, const char* file, int line)
    : time_(Timestamp::now()),
      stream_(),
      level_(level),
      line_(line),
